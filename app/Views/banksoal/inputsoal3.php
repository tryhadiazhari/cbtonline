        ...
        <div class="row my-2">
            <div class="col">
                <h1 style='text-shadow: 2px 2px 4px #827e7e;'>
                    <?= ($mapel['kategori'] == '') ? $titlecontent . " " . $mapel['mapel'] : $titlecontent . " " . $mapel['mapel'] . " " . $mapel['kategori'] ?>
                    <?= (empty($mapel['rombel'])) ? '<small>(' . $mapel['tingkatan'] . ')</small>' : '<small>(' . $mapel['rombel'] . ')</small>' ?>
                </h1>
            </div>
        </div>
    </div>
                        ...
                        <div class="col-12 px-0">
                            <div class="mb-3 text-justify" style="border-bottom: 2px silver solid">
                                <?php if (service('uri')->getSegment(4) == 1) : ?>
                                    ...
                                    <?php endfor; ?>
                                <?php elseif (service('uri')->getSegment(4) == 2) : ?>
                                    <?php for ($i = 1; $i <= $mapel['jlh_essai']; $i++) : ?>
                                        <button type="button" class="btn <?= ($i == service('uri')->getSegment(5)) ? "btn-primary" : "btn-default" ?> btn-xs nomsoal mb-3 mr-1 px-2" data-id="<?= $mapel['uid_banksoal'] ?>" data-soal="<?= $i ?>" data-href="/banksoal/question/<?= $mapel['uid_banksoal'] . '/' . service('uri')->getSegment(4) . '/' . $i ?>"><?= $i ?></button>
                                    <?php endfor; ?>
                                <?php endif ?>
                            </div>
...
<script>
    $(document).ready(function() {
        ...
        function openURL(href) {
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    //     tinymce.get('pil' + value.opsi).setContent(value.jawaban);
                    //     if (value.kunci == 1) {
                    //         $('#flush-' + value.opsi).addClass('show');
                    //         $('#check' + value.opsi).prop('checked', true);
                    //         $('#key' + value.opsi).html('<label class="label label-success rounded-pill">Kunci</label>');
                    //         $('#acc-' + value.opsi).removeClass('collapsed');
                    //     } else {
                    //         $('#acc-' + value.opsi).addClass('collapsed');
                    //         $('#flush-' + value.opsi).removeClass('show');
                    //         $('#check' + value.opsi).prop('checked', false);
                    //         $('#key' + value.opsi).html('');
                    //     }
                    //     $('#file').val('');
                    // });
        ...
        $('form').on('submit', function(e) {
            var url = $('#urls').val();
            var split = url.split('/banksoal/question');
            ...
            $.ajax({
                url: '/banksoal/question/save' + split[1],
                type: "POST",
                cache: false,
                data: formData,
                processData: false,
                ...
                success: function(response) {
                    ...
                    openURL(response.next);
                    $('#urls').val(response.next);
                    $('.nomsoal').removeClass('btn-primary').addClass('btn-default');
                }
            });
        });
