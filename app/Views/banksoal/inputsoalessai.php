        ...
        <div class="row my-2">
            <div class="col">
                <h1 style='text-shadow: 2px 2px 4px #827e7e;'>
                    <?= (empty($mapel['kategori'])) ? $titlecontent . $mapel['mapel'] : $titlecontent . $mapel['mapel'] . " " . $mapel['kategori'] ?> <?= (empty($mapel['rombel'])) ? '<small>(' . $mapel['tingkatan'] . ')</small>' : '<small>(' . $mapel['rombel'] . ')</small>' ?>
                </h1>
            </div>
        </div>
    </div>
                ...
                <div class="card-header px-3">
                    <div class="row">
                        <div class="col-auto ms-auto">
                            <button type="submit" class="btn btn-success bg-gradient btn-sm btn-soal-save">
                                <i class="fas fa-save"></i> <span class="d-none d-lg-inline">Simpan</span>
                            </button>
                            <button type="button" class="btn btn-info bg-gradient btn-sm" onclick="window.location='../'"><i class="fas fa-times"></i></a>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="col-12 px-0">
                            <div class="mb-3 text-justify" style="border-bottom: 2px silver solid">
                                <?php for ($i = 1; $i <= (service('uri')->getSegment(4) == 1 ? $mapel['jml_pg'] : $mapel['jml_esai']); $i++) : ?>
                                    <button type="button" class="btn <?= ($i == service('uri')->getSegment(5)) ? "btn-primary" : "btn-default" ?> bg-gradient btn-xs nomsoal mb-3 mr-1 px-2" data-id="<?= $mapel['uid_banksoal'] ?>" data-soal="<?= $i ?>" data-href="/banksoal/question/<?= $mapel['uid_banksoal'] . '/' . service('uri')->getSegment(4) . '/' . $i ?>"><?= $i ?></button>
                                <?php endfor; ?>
                            </div>
                            <div class="row">
...
<script>
    $(document).ready(function() {
        ...
        function openURL(href) {
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    tinymce.activeEditor.setContent(result.soal);
        ...
        $('form').on('submit', function(e) {
            ...
            $.ajax({
                url: '/banksoal/question/save/' + str[5] + '/2/' + str[7],
                type: "POST",
                cache: false,
                data: formData,
                processData: false,
