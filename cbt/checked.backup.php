...
if ($pageurl[2] == '') {
	...
	if (mysqli_num_rows($cekStatusUjian) > 0) {
		$cekjadwalujian = mysqli_query($koneksi, "SELECT * FROM hazedu_jadwal WHERE tgl_ujian = '" . date('Y-m-d') . "' AND tgl_expired <= '" . date('Y-m-d 23:59:59') . "' AND status = 1");

		if (mysqli_num_rows($cekjadwalujian) > 0) {
			$namamapel = mysqli_query($koneksi, "SELECT * FROM hazedu_banksoal WHERE tingkatan = '" . $siswa['tingkatan'] . "' AND jenjang = '" . $jurusan['jenjang'] . "'");
			$fetchUjian = mysqli_fetch_array($namamapel);

			if (mysqli_num_rows($namamapel) > 0) {
				foreach ($namamapel as $jadwal) {
					if ($jadwal['mapel'] == 'Pendidikan Agama') {
						if ($siswa['agama'] == 'Islam') {
							$jadwalujian = mysqli_query($koneksi, "SELECT * FROM hazedu_jadwal WHERE kode_ujian = '" . $jenisUjian['alias'] . "' AND kategori = '" . $siswa['agama'] . "' AND tingkatan = '" . $siswa['tingkatan'] . "' AND sesi = '" . $siswa['sesi'] . "' AND jenjang = '" . $jurusan['jenjang'] . "' AND `status` = 1 AND tgl_ujian = '" . date('Y-m-d') . "'");
